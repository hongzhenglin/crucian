...
public class IPAddress implements Cloneable {
	 ...
	 *                Throws this exception if the specified IP address is not
	 *                compliant to the decimal-dotted notation xxx.xxx.xxx.xxx.
	 */
			...
			for (int i = 0; i < 3; i++) {
				...
				if (index != -1) {
					...
					if ((number < 0) || (number > 255)) {
						throw new IllegalArgumentException(
								"Invalid IP Address [" + ipAddressStr + "]");
					}
	...
	public boolean equals(Object another) {
		return another instanceof IPAddress
				&& ipAddress == ((IPAddress) another).ipAddress;
	}
}
